...
from algos.functions import random_choice
from algos.tree import Tree
import numpy as np


def select(tree:Tree, tree_idx, f_ucb):
    '''Return the index of the selected nodes.'''
...
def expand(tree:Tree, tree_idx, f_actions, f_terminal):
    '''Return the index of the randomly expanded nodes.'''
...
def roll_out(tree:Tree, tree_idx, f_actions, f_sampler, f_terminal):
...
def backprop(tree:Tree, tree_idx, current_score, _):
...
def growTree(tree:Tree, tree_idx, f_score, f_actions, f_terminal, f_ucb, f_sampler, f_backprop=backprop):
    '''MCTS growTree. Return the rollout score and history.'''
